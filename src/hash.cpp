...
static void do_hash_buffer(struct hash* hash, const void* s, size_t len)
{
  assert(s);
...
static void do_debug_text(struct hash* hash, const void* s, size_t len)
{
...
bool hash_fd(struct hash* hash, int fd, bool fd_is_file)
{
